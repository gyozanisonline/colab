...
function love.load()
	...
	-- Skip platform-specific code on desktop
...
function love.update( dt )
	...
	if shouldSplash then
		if love.platform and love.platform.hideSplashScreen then
...
function love.draw()
	...
	if splashVideo and not shouldSplash then
		...
		love.event.pump()
		if not splashVideo:isPlaying() or (love.platform and love.platform.anyButtonPressed and love.platform.anyButtonPressed() and love.platform.isFirstTimePlaying and not love.platform.isFirstTimePlaying()) then
			G:start_up()
			started = true
			splashVideo = nil
		end
