class KineticLetter {
    constructor(x_, p_, m_, n_) {
        ...
        this.x0 = x_;
        this.y0 = 0;
        this.w = textWidth(snap_keyArray[this.m].charAt(this.n));
        this.h = snap_pgTextSize * 0.7;
        ...
        if (this.n == 0) {
            ...
            this.yAmax = 0;
        } else if (this.n == snap_keyArray[this.m].length - 1) {
            this.xAmax = random(40, 160);
            this.yAmax = 0;
        ...
        var rs0 = random(100);
        if (rs0 < 100 * (1 / snap_keyArray[this.m].length)) {
            this.xShearMaxB = random(-PI / 4, PI / 4);
    ...
    update() {
        ...
        if (this.n < snap_keyArray[this.m].length - 1) {
            snap_kineticGroups[this.p].kineticWords[this.m].budgeCenter += (this.xBudgeScale + this.xBudgePost + this.xBudgePre);
        }
        ...
        if (this.ticker > this.anim23 - 1) {
            ...
            if (this.m == snap_keyArray.length - 1 &&
                this.n == snap_keyArray[this.m].length - 1 &&
                this.p == snap_groupCount - 1) {
                snap_resetAnim();
            }
        }
    ...
    display() {
        ...
        if (this.visible) {
            ...
            noStroke();
            fill(snap_foreColor);
            textFont(snap_currentFont);
            textSize(snap_pgTextSize);
            text(snap_keyArray[this.m].charAt(this.n), 0, 0);
