...
function Platform:unlockAchievement(gameId)
    ...
    if achievementId ~= nil then
        -- Only unlock achievements if platform module is available
        if love.platform and love.platform.unlockAchievement then
            love.platform.unlockAchievement(achievementId)
        else
            print("[DESKTOP] Achievement unlocked: "..tostring(gameId))
        end
    else -- retail-remove
