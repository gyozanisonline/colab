...
class PosterGeometry extends Geometry {
    constructor(width = 2, height = 3) {
        ...
        this.addFace(0, 1, 2); // Bottom-left, bottom-right, top-right
        this.addFace(0, 2, 3); // Bottom-left, top-right, top-left
    }
}
...
class InfiniteGridMenu {
    ...
    run(time = 0) {
        ...
        // Skip expensive rendering when tab is hidden
        if (document.hidden) {
        ...
        if (this.videoElements) {
            this.videoElements.forEach(el => {
                if (el && el.tagName === 'VIDEO') {
                    el.pause();
                    el.removeAttribute('src'); // help gc
                    el.load();
                }
            });
    ...
    #init(onInit) {
        ...
        this.discGeo = new PosterGeometry(2, 3); // 2:3 aspect ratio for posters
                ...
                (item, index) =>
                    new Promise((resolve) => {
                        ...
                        if (item.video) {
                            ...
                            img.crossOrigin = 'anonymous';
                            img.onload = () => {
                                resolve({ element: img, index });
                            };
                            img.onerror = () => {
                                console.error(`Image ${index} failed:`, item.image);
                                resolve({ element: img, index });
                            };
                            img.src = item.image;
                            this.videoElements[index] = null;
    ...
    #updateVideoPlayback(activeIndex) {
        ...
        this.videoElements.forEach((video, index) => {
            ...
            if (index === activeIndex) {
                if (video.paused) {
                    video.play().catch(err => console.error(`Video ${index} play failed:`, err));
                }
...
const defaultItems = [
    {
        image: 'https://placehold.co/600x900/111/fff?text=Poster',
        link: '',
        title: 'Community',
